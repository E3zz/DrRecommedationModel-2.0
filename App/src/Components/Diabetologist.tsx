...
const Diabetologist = () => {
  ...
  const fetchData = () => {
    setLoading(true);
    fetch("http://127.0.0.1:5000/Diabetes")
  ...
  return (
    <>
      <div className="recommendation-container-Diabetologist">
        <div className="secondary-gyn">
          <h1>Personalized diabetes care from top-rated Diabetologists.</h1>
        ...
        >
          <div className="sec-con">
            ...
            <div className="bio">
              ...
              <p>
                {`${item.Doctor_Name} is a skilled diabetologist with over `}
                <strong>{item.Experience}</strong>
                {` years of experience in managing diabetes and related metabolic disorders. They have helped over `}
                <strong>{item.Number_Of_Patients}</strong>
                {` patients and consistently maintained a `}
                <strong>{item.Satisfaction_level}%</strong>
                {` satisfaction rating. ${item.Doctor_Name} focuses on personalized diabetes care, lifestyle management, and prevention of complications.`}
              </p>
            </div>
