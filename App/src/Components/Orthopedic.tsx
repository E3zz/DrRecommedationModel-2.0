...
const Orthopedic = () => {
  ...
  const fetchData = () => {
    setLoading(true);
    fetch("http://127.0.0.1:5000/orthopedic")
  ...
  return (
    <>
      <div className="recommendation-container-ortho">
        <div className="secondary-ortho">
          <h1>Best Orthopedics In Town.</h1>
        ...
        >
          <div className="sec-con">
            ...
            <div className="bio">
              ...
              <p>
                {`${item.Doctor_Name} is a highly trained orthopedic specialist with over `}
                <strong>{item.Experience}</strong>
                {` years of experience in treating musculoskeletal issues. They have provided care to more than `}
                <strong>{item.Number_Of_Patients}</strong>
                {` patients and achieved a `}
                <strong>{item.Satisfaction_level}%</strong>
                {` satisfaction rating. ${item.Doctor_Name} specializes in joint replacement, sports injuries, spinal care, and fracture management.`}
              </p>
            </div>
